...
- `void aJobSitelessVillagerNeverEntersWorkButAnEmployedOneDoes(GameTestHelper helper)` — `VC-21`'s own acceptance criterion, stated directly: a job-siteless villager can never be forced into Activity.WORK at all, and an employed one still can.
- `void employAsFarmer(GameTestHelper helper, ServerLevel level, Villager villager, BlockPos composterPos)` — Employs villager as a FARMER with the composter at composterPos (which must already exist as a block — a POI record only exists for a real one) as its real JOB_SITE (`VC-21`): sets its profession, rebuilds its brain (which nulls its offers — callers must re-add them afterwards), claims the composter POI directly through PoiManager.take (its own entry condition, VillagerProfession.heldJobSite(), is a bare POI-type predicate with no ownership check, confirmed via javap -p -c) and sets JOB_SITE to it — rather than waiting on vanilla's own AssignProfessionFromJobSite/AcquirePoi, both timing-dependent and not any of these tests' own concern.
