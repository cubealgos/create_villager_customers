...
| `villager_customers.customer` | root | CustomerHooks, CustomerMemoryModules, CustomerRegistration, CustomerRules, ShoppingTripBehavior |  |
...
| `villager_customers.customer` | root (test) | CustomerRulesTest |  |
...
| `tools/icon.py` | Render docs/modrinth/icon.png: a villager on the round blueprint badge Create's add-ons share. | `badge()`, `subject(img)`, `main()` |
