...
| `CUSTOMER-REQ-003` | When the roll succeeds, the system shall search the village's shop POIs (`domains/shop.md`) for one whose goods and price match any of the villager's offers with uses remaining, and remember the chosen one's position as the trip target. | Must | `UC-001` |
