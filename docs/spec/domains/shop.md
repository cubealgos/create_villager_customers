...
| `SHOP-REQ-004` | The system shall restrict a villager's shop search to POIs within its own village; no distance beyond vanilla's own village/POI search reach is added. The exact figure is confirmed at the first ticket. | Must | `rulings-2026-09-19.md` **Amended (`DEC-010`, Kevin, 2026-09-20):** withdrawn as written; the search is village-wide, 128 blocks from the meeting point, config `shop_search_radius`. |
