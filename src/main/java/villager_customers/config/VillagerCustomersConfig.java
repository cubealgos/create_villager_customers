...
/**
 * The mod's one config value, {@code shop_search_radius}
 * (`docs/spec/decisions/DEC-010-village-wide-shop-search.md`, `CUSTOMER-REQ-003`), read once at
 * startup from {@code villager_customers.properties} under Fabric's own config directory. Backed
 * by {@link Properties} — the JDK's own key/value file format — rather than a hand-rolled parser
 * or a new config-library dependency for one clamped integer
 ...
 * amended by `VC-18`).
 *
 * <p>{@link #load()} always (re)writes the file with the doc comment and the clamped value it is
 * about to use: an absent file gets created with the default, and an out-of-range or malformed
...
public final class VillagerCustomersConfig {
    ...
    static void load(Path path) {
        ...
        int clamped = CustomerRules.clampShopSearchRadius(parseRadius(properties.getProperty(KEY_SHOP_SEARCH_RADIUS)));
        shopSearchRadius = clamped;
        writeNormalized(path, clamped);
    }
    ...
    private static void writeNormalized(Path path, int value) {
        Properties properties = new Properties();
        properties.setProperty(KEY_SHOP_SEARCH_RADIUS, String.valueOf(value));
        try {
