 ...
 * (`docs/spec/decisions/DEC-010-village-wide-shop-search.md`, `CUSTOMER-REQ-003`) and
 * {@code keeper_seek_cooldown_ticks} (`docs/spec/domains/keeper.md` `KEEPER-REQ-004`, `VC-20`,
 * added at the end per that ticket's own Build step 4) — read once at startup from
 ...
 * new config-library dependency for a couple of clamped integers
...
public final class VillagerCustomersConfig {
    ...
    static void load(Path path) {
        ...
        writeNormalized(path, clampedRadius, clampedCooldown);
    }
    ...
    private static void writeNormalized(Path path, int shopSearchRadiusValue, int keeperSeekCooldownTicksValue) {
        Properties properties = new Properties();
